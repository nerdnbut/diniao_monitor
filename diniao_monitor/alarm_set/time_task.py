...
def send_email(subject, message, recipient_email):
    """发送电子邮件通知"""
    sender_email = formataddr(["Diniao", '312121231@qq.com'])  # 填写你的发送邮箱
    sender_password = "icwrewrvvhdafdcjch"  # 填写你的邮箱密码
    ...
    try:
        server = smtplib.SMTP(smtp_server, 465)  # SMTP端口号
        server.starttls()  # 启用TLS加密
        server.login("312121231@qq.com", sender_password)
        server.sendmail("312121231@qq.com", recipient_email, msg.as_string())
        server.quit()
...
def trigger_alarm(task, message):
    ...
    recipient_email = "1234567@qq.com"  # 填写接收报警的邮箱
...
def fetch_server_data_and_check_alarms():
    print('开始报警任务')
    # 获取所有报警任务
    alarm_tasks = AlarmTask.objects.all()
...
# 运行fetch_server_data_and_check_alarms函数
if __name__ == '__main__':
    fetch_server_data_and_check_alarms()
